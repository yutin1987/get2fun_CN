...
Task = function(e) {
    ...
    $.task.on("add", function(e) {
        ...
        $(".percentage .bar", t).width(100 * (e.get("dlSize") / (e.get("size") || 1)) + "%"), 
        $(".percentage .num", t).text(Math.floor(1e3 * (e.get("dlSize") / (e.get("size") || 1))) / 10 + "%"), 
        $(".add_time span", t).text(new Date(1e3 * e.get("added_time")).toFormat("yyyy-MM-dd")), 
        e.dom = t, s.append(t);
    }), $.task.on("change:dlSize", function(e, t) {
        return $(".percentage .bar", e.dom).width(100 * (t / (e.get("size") || 1)) + "%"), 
        $(".percentage .num", e.dom).text(Math.floor(1e3 * (t / (e.get("size") || 1))) / 10 + "%");
